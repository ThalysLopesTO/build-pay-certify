-- Fix the approve_missed_punch_request function to handle timezone correctly
...
BEGIN
  ...
  CASE request_record.punch_type
    WHEN 'in' THEN
      -- Check if timesheet exists for this date and user
      SELECT * INTO existing_timesheet
      FROM timesheets
      WHERE user_id = request_record.user_id 
        AND DATE(check_in_time AT TIME ZONE 'UTC') = request_record.punch_date
        AND company_id = request_record.company_id;
      
      IF FOUND THEN
        -- Update existing timesheet with corrected in time (keep timezone as is from request)
        UPDATE timesheets
        SET 
      ...
      ELSE
        -- Create new timesheet with in time (keep timezone as is from request)
    ...
    WHEN 'out' THEN
      ...
      IF FOUND THEN
        -- Update with corrected out time (keep timezone as is from request)
        UPDATE timesheets
        SET 
    ...
    WHEN 'both' THEN
      -- Find existing timesheet for the date
      SELECT * INTO existing_timesheet
      FROM timesheets
      WHERE user_id = request_record.user_id 
        AND DATE(check_in_time AT TIME ZONE 'UTC') = request_record.punch_date
      ...
      IF FOUND THEN
        -- Update existing timesheet with both corrected times (keep timezone as is from request)
        UPDATE timesheets
        SET 
          check_in_time = request_record.corrected_in_time,
          check_out_time = request_record.corrected_out_time,
        ...
        WHERE id = existing_timesheet.id;
      ELSE
        -- Create new timesheet with both times (keep timezone as is from request)
        INSERT INTO timesheets (
          user_id,
          company_id,
          check_in_time,
