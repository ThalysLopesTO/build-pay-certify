-- Clean up orphaned records and then update foreign key constraints

-- First, clean up orphaned records in employee_certificates
DELETE FROM public.employee_certificates 
WHERE employee_id NOT IN (SELECT user_id FROM public.user_profiles);

-- Clean up orphaned records in other tables
DELETE FROM public.employee_certificates 
WHERE uploaded_by NOT IN (SELECT user_id FROM public.user_profiles);

DELETE FROM public.audit_logs 
WHERE edited_by_user_id NOT IN (SELECT user_id FROM public.user_profiles);

DELETE FROM public.audit_logs 
WHERE employee_id NOT IN (SELECT user_id FROM public.user_profiles);

DELETE FROM public.weekly_timesheet_audit_logs 
WHERE edited_by_user_id NOT IN (SELECT user_id FROM public.user_profiles);

DELETE FROM public.attention_reports 
WHERE submitted_by NOT IN (SELECT user_id FROM public.user_profiles);

DELETE FROM public.attention_reports 
WHERE reviewed_by IS NOT NULL AND reviewed_by NOT IN (SELECT user_id FROM public.user_profiles);

DELETE FROM public.material_requests 
WHERE submitted_by NOT IN (SELECT user_id FROM public.user_profiles);

DELETE FROM public.material_takeoff_notes 
WHERE created_by NOT IN (SELECT user_id FROM public.user_profiles);

DELETE FROM public.material_takeoff_notes 
WHERE updated_by IS NOT NULL AND updated_by NOT IN (SELECT user_id FROM public.user_profiles);

DELETE FROM public.bills_expenses 
WHERE created_by NOT IN (SELECT user_id FROM public.user_profiles);

DELETE FROM public.safety_templates 
WHERE uploaded_by NOT IN (SELECT user_id FROM public.user_profiles);

DELETE FROM public.inventory 
WHERE created_by NOT IN (SELECT user_id FROM public.user_profiles);

DELETE FROM public.jobsite_tasks 
WHERE created_by NOT IN (SELECT user_id FROM public.user_profiles);
...
WHERE admin_user_id NOT IN (SELECT user_id FROM public.user_profiles);

DELETE FROM public.password_reset_logs 
WHERE target_user_id NOT IN (SELECT user_id FROM public.user_profiles);

DELETE FROM public.cancellation_requests 
WHERE requested_by NOT IN (SELECT user_id FROM public.user_profiles);

DELETE FROM public.timesheets 
WHERE user_id NOT IN (SELECT user_id FROM public.user_profiles);

-- Now update foreign key constraints to allow employee deletion while preserving historical data
