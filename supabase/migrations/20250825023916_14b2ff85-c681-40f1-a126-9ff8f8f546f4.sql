-- Fix bi-weekly timesheet totals that don't match calculated daily hours
-- This migration recalculates total_hours from bi-weekly JSON data

CREATE OR REPLACE FUNCTION fix_biweekly_timesheet_totals()
RETURNS TABLE(timesheet_id uuid, old_total_hours numeric, new_total_hours numeric, hours_fixed boolean)
LANGUAGE plpgsql
AS $$
DECLARE
...
SELECT * FROM fix_biweekly_timesheet_totals();