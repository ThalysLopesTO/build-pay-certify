-- Fix get_client_portal_data function: Move ORDER BY inside json_agg() calls
-- This fixes the SQL error: "column must appear in the GROUP BY clause or be used in an aggregate function"

CREATE OR REPLACE FUNCTION public.get_client_portal_data(p_portal_token UUID)
RETURNS JSON
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  ...
  v_result JSON;
BEGIN
  -- Get client_id and company_id from the portal_token
