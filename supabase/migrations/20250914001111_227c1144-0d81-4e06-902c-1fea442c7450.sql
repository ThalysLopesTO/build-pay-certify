-- Enhanced Certificate Expiry Notification System

-- First, update the existing check_expiring_certificates function to include foremen and expired certificates
...
-- Create function to handle immediate certificate notifications on insert/update
...
-- Create trigger for immediate notifications on certificate insert/update
DROP TRIGGER IF EXISTS certificate_status_notification_trigger ON public.employee_certificates;
CREATE TRIGGER certificate_status_notification_trigger
  AFTER INSERT OR UPDATE OF expiry_date, certificate_type, certificate_name
  ON public.employee_certificates
  FOR EACH ROW
  EXECUTE FUNCTION public.notify_certificate_status_change();