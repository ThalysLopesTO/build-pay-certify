...
import { Calendar, Clock, CheckCircle, AlertCircle, Play, Pause, Edit, Trash2 } from 'lucide-react';
...
const JobsiteTaskCard: React.FC<JobsiteTaskCardProps> = ({ task, isAdmin }) => {
  ...
  const isOverdue = task.status !== 'completed' && isPast(new Date(task.end_date));
          ...
          <div className="flex-1">
            ...
            <div className="flex items-center gap-4 text-sm text-gray-600 mb-3">
              <div className="flex items-center gap-1">
                ...
                {format(new Date(task.start_date), 'MMM dd, yyyy')} - {format(new Date(task.end_date), 'MMM dd, yyyy')}
              </div>
              <div className="flex items-center gap-1">
