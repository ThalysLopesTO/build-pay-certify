...
export const EnhancedTaskItem: React.FC<EnhancedTaskItemProps> = ({
  ...
  return (
    <div
      className={cn(
        'group py-2 px-3 rounded-md hover:bg-accent/50 transition-all',
        task.is_done && 'opacity-60',
        indentLevel > 0 && 'ml-8'
      )}
      style={{ paddingLeft: indentLevel > 0 ? `${indentLevel * 32}px` : undefined }}
    >
      <div className="flex items-start gap-3">
        {/* Priority Dot */}
        {task.priority && (
          <div className="flex-shrink-0 mt-1.5">
            <div className={cn('w-2 h-2 rounded-full', priorityDotClass)} />
          </div>
        )}
        ...
        <div className="mt-1">
          {task.subtasks.map((subtask) => (
            <EnhancedTaskItem
              key={subtask.id}
              task={subtask}
              assignees={subtask.daily_task_item_assignees || []}
              tags={subtask.daily_task_item_tags || []}
              canEdit={canEdit}
              onToggle={onToggle}
              onUpdate={onUpdate}
              onDelete={onDelete}
